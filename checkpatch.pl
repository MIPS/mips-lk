...
if ($git) {
	...
	foreach my $commit_expr (@ARGV) {
		...
		my $lines = `git log --no-color --pretty=format:'%H %s' $git_range`;
...
# ignore non-hunk lines and lines being removed
		...
		next if ($realfile =~ /^external/);
		next if ($realfile =~ /^platform/);
...
# mips-lk: only for files in trusty/
		if ($realfile =~ /^trusty/) {
			if ($rawline =~ /^\+\s* \t\s*\S/ ||
			    $rawline =~ /^\+\s*        \s*/) {
				my $herevet = "$here\n" . cat_vet($rawline) . "\n";
...
# mips-lk/trusty
		if ($^V && $^V ge 5.10.0 && $realfile =~ /^trusty/ &&
